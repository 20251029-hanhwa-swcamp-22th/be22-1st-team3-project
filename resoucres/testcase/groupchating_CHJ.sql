	/*8	그룹 채팅	[생성] '사용자 A'가 '그룹 채팅' 탭으로 이동합니다.
			채팅방 만들기'를 통해 '무릎 통증 환우회'라는 이름의 채팅방을 개설합니다.
			[초대] (테스트용) '사용자 B'를 검색하여 채팅방에 초대합니다.
			[메시지 전송] 채팅방에서 "오늘 스쿼트했더니 무릎이 아프네요"라고 텍스트 메시지를 전송합니다.
			[신고] (테스트 시나리오) '사용자 B'가 부적절한 메시지나 이미지를 전송한 것을 가정하고, '사용자 A'가 해당 메시지를 길게 눌러 '신고하기'를 선택합니다.
		[퇴장] '사용자 A'가 채팅방 설정 메뉴에서 '채팅방 나가기'를 실행합니다.		*/
